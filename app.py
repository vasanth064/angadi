...
class Sale(db.Model):
    ...
    store_id = db.Column(db.Integer, db.ForeignKey('store.id'), nullable=False)
    sale_number = db.Column(db.Integer, nullable=False)  # Store-specific sale number
    total_amount = db.Column(db.Float, nullable=False)
    discount = db.Column(db.Float, default=0)
    timestamp = db.Column(db.DateTime, default=current_timestamp)
    customer_name = db.Column(db.String(100), nullable=False, default="Unknown")
    customer_phone = db.Column(db.String(20), nullable=False, default="Not Provided")

    # Ensure uniqueness of sale_number per store
    __table_args__ = (db.UniqueConstraint('store_id', 'sale_number', name='unique_store_sale_number'),)
...
def add_product():
    ...
    new_product = Product(store_id=session['user_id'], name=name, price=price, stock=stock, hidden=hidden)
...
def pos():
    ...
    products = Product.query.filter_by(store_id=session['user_id'], hidden=False).all()  # Exclude hidden products
...
def process_sale():
    ...
    total_amount = sum(float(item['price']) * int(item['quantity']) for item in items) - float(discount)
    
    # Get the latest sale number for the store
    last_sale = Sale.query.filter_by(store_id=store_id).order_by(Sale.sale_number.desc()).first()
    new_sale_number = (last_sale.sale_number + 1) if last_sale else 1  # Start from 1 if no previous sale
    ...
    for item in items:
        ...
        if product and product.stock >= int(item['quantity']):
            ...
            sale_item = SaleItem(sale_id=sale.id, product_id=product.id, quantity=int(item['quantity']), price=float(product.price))
...
def sales_report():
    ...
    for sale in sales:
        ...
        sale.items = SaleItem.query.filter_by(sale_id=sale.id).all()  # Sale items with related product
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not Store.query.filter_by(is_admin=True).first():
            admin = Store(name='Admin', username='admin', password='admin123', is_admin=True)
